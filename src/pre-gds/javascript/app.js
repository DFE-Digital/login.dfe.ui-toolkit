...
NSA = {
  showPassword: function () {
    ...
    $showPassword.each(function (i) {
      ...
      var $checkLbl = $("<label />")
        .prop({ for: "show-password-" + i })
        .html("Show");
      var $checkBox = $("<input />")
        .prop({ type: "checkbox", id: "show-password-" + i })
        .on("change", function () {
          var $that = $(this);
      ...
      var $cbWrap = $("<div />")
        .prop("class", "show-password-control")
        .append($checkBox, $checkLbl);
      $pwdFld.after($cbWrap);
    });
  },
  backLink: function () {
...
$("#add-redirect").on("click", function () {
  $(
    '<p><label for="redirect-url"><input class="form-control inputConfig" name="redirect_uris"></label></p>',
  ).appendTo(redirect);
  return false;
});
...
$("#add-logout").on("click", function () {
  $(
    '<p><label for="logout-url"><input class="form-control inputConfig" name="post_logout_redirect_uris"></label></p>',
  ).appendTo(logout);
  return false;
});
