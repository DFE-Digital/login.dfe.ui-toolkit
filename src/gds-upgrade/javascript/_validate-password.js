function validatePassword() {
  ...
  $(`#length-${id}`).removeClass(password.length >= 14 ? "icon-remove" : "icon-ok");
  $(`#length-${id}`).addClass(password.length >= 14 ? "icon-ok" : "icon-remove");
}
...
$('.password-input-check').each(
  function(index, element) {
    ...
    .append(`<div id='length-${id}' class='icon-remove'>be at least 14 characters</div>`);
